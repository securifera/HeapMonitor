...
void * __cdecl mallocHook(size_t size){
	...
	if( profiling /**&& shouldHook **/ ){
		...
		//Get buf data
		retPtr = aMsg.getBytes( &retBytes );
		fprintf(pFile, "Return pointer %p, size %d.\n", retPtr, retBytes);
		if( retPtr && retBytes > 0 && ClientSocket ){
			
			//Send the bytes 
			int sent_count = send(ClientSocket, (const char *)retPtr, retBytes, 0 );
			if(pFile)
...
void  __cdecl freeHook(void * p){
	...
	if( profiling /**&& shouldHook **/){
		...
		if( retPtr && retBytes > 0 && ClientSocket ){
			...
			//Send the bytes 
			send(ClientSocket, (const char *)retPtr, retBytes, 0 );
			if(pFile)
